...
public class PreferredServices implements com.android.nfc.ForegroundUtils.Callback {
    ...
    public interface Callback {
        // [SEC] Get AID overflow status by return value
        boolean onPreferredPaymentServiceChanged(ComponentName service);
    ...
    void loadDefaultsFromSettings(int userId) {
        ...
        // [SEC] Rollback to old value if AID overflow is happened
        ComponentName oldDefault = mPaymentDefaults.settingsDefault;
        try {
        ...
        if (paymentDefaultChanged) {
            if (!mCallback.onPreferredPaymentServiceChanged(newDefault)) {
                Log.d(TAG, "AID Overflow back to: " + oldDefault);
                if (oldDefault != null) {
                    mPaymentDefaults.currentPreferred = oldDefault;
                    Settings.Secure.putString(mContext.getContentResolver(),
                                              Settings.Secure.NFC_PAYMENT_DEFAULT_COMPONENT,
                                              oldDefault != null ? oldDefault.flattenToString() : null);
                }
                paymentPreferForegroundChanged = false;
            }
