...
bool gIsMifareSelectingRfInterface = false;
bool gIsMultiProtocolTag = false;
}  // namespace android
...
static bool sIsTagDeselectState = false;
...
void nativeNfcTag_doConnectStatus(jboolean isConnectOk) {
  if (gIsMultiProtocolTag &&
      gIsSelectingRfInterface &&
      (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP) &&
      (NfcTag::getInstance ().getActivationState () != NfcTag::Active)) {
    NfcTag::getInstance().setSkipTagDetectionNotify(true);
    LOG(ERROR) << StringPrintf("%s: RF switching error (Type1)..!!", __FUNCTION__);
    return;
  }
  ...
  if (EXTNS_GetConnectFlag() == TRUE && legacy_mfc_reader) {
    ...
    gIsMifareSelectingRfInterface = false;
    return;
  }
...
void nativeNfcTag_doDeactivateStatus(int status) {
  if (EXTNS_GetDeactivateFlag() == TRUE && legacy_mfc_reader) {
    ...
    gIsMifareSelectingRfInterface = true;
    return;
  }
...
static jint nativeNfcTag_doConnect(JNIEnv*, jobject, jint targetHandle) {
  ...
  if (natTag.mNumDiscTechList > 3)
    gIsMultiProtocolTag = true;
  else
    gIsMultiProtocolTag = false;

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: natTag.mNumDiscTechList = 0x%02X ", __FUNCTION__, natTag.mNumDiscTechList);
...
static int reSelect(tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded) {
  ...
  do {
    ...
    if (NfcTag::getInstance().getActivationState() != NfcTag::Active) {
      ...
      if (NfcTag::getInstance().getSkipTagDetectionNotify()) {
        SyncEventGuard g (sReconnectEvent);
        sConnectWaitingForComplete = JNI_FALSE;
        LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: deactivate to idle", __func__);
        sConnectOk = true;

        if (sReconnectEvent.wait (1000) == false) { //if timeout occurred
          LOG(ERROR) << StringPrintf("%s: timeout waiting for deactivate", __func__);
          sConnectWaitingForComplete = JNI_TRUE;
          NfcTag::getInstance().setSkipTagDetectionNotify(false);
        }
      } else {
        rVal = STATUS_CODE_TARGET_LOST;
        break;
...
void nativeNfcTag_doTransceiveStatus(tNFA_STATUS status, uint8_t* buf,
  ...
  if (sIsTagDeselectState)
    sIsTagDeselectState = false;

  if (sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP) {
    if (bufLen == 0x02) {     // Response Length
      if (buf[0] == 0xC2)      // Response Packet Header
        sIsTagDeselectState = true;
    }
  }
...
static jbyteArray nativeNfcTag_doTransceive(JNIEnv* e, jobject o,
  ...
  if ((sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP) && sIsTagDeselectState) {
    if(sCurrentRfInterface == NFA_INTERFACE_FRAME)
      sCurrentRfInterface = NFA_INTERFACE_ISO_DEP;

    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Re-select RF interface because of C2 tag status..!!", __func__);
    reSelect(sCurrentRfInterface, false);
  }
...
static jboolean nativeNfcTag_doPresenceCheck(JNIEnv*, jobject) {
  ...
  if (!gActivated) {
    DLOG(ERROR) << StringPrintf ("%s: skip presence check. (Call a doPresencecheck after complete connection", __FUNCTION__);
    return JNI_FALSE;
  }
  ...
  if (sCurrentConnectedTargetProtocol == NFC_PROTOCOL_MIFARE && legacy_mfc_reader) {
    ...
    if(status == NFCSTATUS_SUCCESS) {
      ...
      isPresent = JNI_FALSE;
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tag absent", __func__);
    }
    return isPresent;
#endif
  }
  if ((sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP) &&
      ...
      (sIsTagDeselectState == true)) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf ("%s: Do RF switch for checking Tag presence.", __FUNCTION__);
    sIsTagDeselectState = false;
    status = reSelect(NFA_INTERFACE_ISO_DEP, false);
