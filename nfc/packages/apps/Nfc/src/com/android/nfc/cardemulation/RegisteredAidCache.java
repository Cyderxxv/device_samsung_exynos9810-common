...
public class RegisteredAidCache {
    ...
    boolean mIsOpenAidOverflowDialog = false; // [SEC] Preventing duplicate dialog
    ...
    void updateRoutingLocked(boolean force) {
        ...
        // [SEC] AID overflow handling
        if (!mRoutingManager.configureRouting(routingEntries, force)) {
            if (mRoutingManager.isNeedToUpdateRoute()) {
                generateAidCacheLocked(); // Update route info
            }
            if (mRoutingManager.isAidOverflow()) {
                notifyAidOverflow(); // Handle both overflow
            }
        }
    ...
    // [SEC] Return AID overflow state
    public boolean onPreferredPaymentServiceChanged(ComponentName service) {
        if (DBG) Log.d(TAG, "Preferred payment service changed.");
        boolean isOverflow;
       synchronized (mLock) {
           ComponentName currentPaymentService = mPreferredPaymentService;
           mPreferredPaymentService = service;
           generateAidCacheLocked();
           isOverflow = mRoutingManager.isAidOverflow();
           if (isOverflow) { // [SEC] AID overflow happen back to original one
               if (DBG) Log.d(TAG, "Back to current preferred payment service");
               mPreferredPaymentService = currentPaymentService;
           }
       }
       return isOverflow; // [SEC]
    }
    ...
    private void notifyAidOverflow() {
        Log.i(TAG, "notifyAidOverflow, user should select others to use");
        mRoutingManager.removeAids();

        if (getServiceDescription() == null) { // Don't need to inform to user
            return;
        }
        ...
        Runnable myRunnable = new Runnable() {
            ...
            public void run() {
                ...
                builder.setTitle(mContext.getString(com.android.nfc.R.string.aid_overflow_dialog_title));
                builder.setMessage(mContext.getString(com.android.nfc.R.string.aid_overflow_dialog_message, getServiceDescription()));
                builder.setPositiveButton(mContext.getString(android.R.string.ok), new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        mIsOpenAidOverflowDialog = false;
                        dialog.dismiss();
                    }
                });
                ...
                if (!mIsOpenAidOverflowDialog) {
                    mIsOpenAidOverflowDialog = true;
                    alert.show();
                }
            }
        };
