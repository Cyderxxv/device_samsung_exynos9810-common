...
extern bool gIsMifareSelectingRfInterface;
extern bool gIsMultiProtocolTag;
  ...
  switch (connEvent) {
    ...
    case NFA_SELECT_RESULT_EVT:  // NFC link/protocol discovery select response
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          ...
          "%d, gIsMifareSelectingRfInterface = %d"
          "sIsDisabling=%d",
          __func__, eventData->status, gIsSelectingRfInterface, gIsMifareSelectingRfInterface, sIsDisabling);

      if (sIsDisabling) break;

      if (eventData->status != NFA_STATUS_OK) {
        if (gIsSelectingRfInterface || gIsMifareSelectingRfInterface) {
        ...
        if ((NfcTag::getInstance().getSkipTagDetectionNotify() && gIsMultiProtocolTag)) {
    ...
    case NFA_ACTIVATED_EVT:  // NFC link/protocol activated
      ...
      if (isPeerToPeer(eventData->activated)) {
        ...
        if (NfcTag::getInstance ().mNumDiscNtf) {
          LOG(ERROR) << StringPrintf("%s: Multi-Protocol Tag detected, go to sleep", __func__);
          NFA_Deactivate (true);
        }
...
static jint nfcManager_doGetDefaultRoute(JNIEnv* e, jobject o) {
  RoutingManager& routingManager = RoutingManager::getInstance();
  return routingManager.getDefaultEe();
}
