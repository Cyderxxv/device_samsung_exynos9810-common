...
#define DISC_PROTOCOL_T1T       0x0001
#define DISC_PROTOCOL_T2T       0x0002
#define DISC_PROTOCOL_T3T       0x0004
#define DISC_PROTOCOL_ISO_DEP   0x0008
#define DISC_PROTOCOL_NFC_DEP   0x0010
#define DISC_PROTOCOL_MIFARE    0x0020
#define DISC_PROTOCOL_15693     0x0040
#define DISC_PROTOCOL_BPRIME    0x0080
#define DISC_PROTOCOL_KOVIO     0x0100
#define DISC_PROTOCOL_CLT       0x0200
#define DISC_PROTOCOL_UNKNOWN   0x1000

uint16_t gDiscoverProtocols;
uint16_t gLastDiscoverProtocols;
tNFC_INTF_PA_ISO_DEP gMulti_pa_iso;

static uint16_t transProtocolValue(tNFC_PROTOCOL protocol);
...
bool NfcTag::isActivated() { return mIsActivated; }
  ...
  /*******************************************************************************
  **
  ** Function:        setSkipTagDetectionNotify
  **
  ** Description:
  **
  ** Returns:         None
  **
  *******************************************************************************/
void NfcTag::setSkipTagDetectionNotify (bool isSkip) {
  mIsSkipNotification = isSkip;
}

  /*******************************************************************************
  **
  ** Function:        getSkipTagDetectionNotify
  **
  ** Description:
  **
  ** Returns:         None
  **
  *******************************************************************************/
bool NfcTag::getSkipTagDetectionNotify (void) {
  return mIsSkipNotification;
}
...
void NfcTag::discoverTechnologies(tNFA_ACTIVATED& activationData) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: mNumTechList =%d, mTechListIndex=%d",
        fn, mNumTechList, mTechListIndex);
  ...
  gDiscoverProtocols |= transProtocolValue(rfDetail.protocol);
  mNumTechList++;
...
void NfcTag::discoverTechnologies(tNFA_DISC_RESULT& discoveryData) {
  ...
  if (mIsSkipNotification) {
    mIsSkipNotification = false;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s; Discovered a multi-protocol tag (TechList=%x)", fn, mNumDiscTechList);
  } else {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s; Re-Discovered an abnormal multi-protocol tag (TechList=%x)", fn, mNumDiscTechList);
  }
  }
...
void NfcTag::createNativeNfcTag(tNFA_ACTIVATED& activationData) {
  ...
  for (int i=0; i < mNumDiscTechList; i++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: index=0x%x; tech=0x%x; handle=0x%x", fn, i, mTechList[i], mTechHandles[i]);
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: index=0x%x; type=0x%x; sak=0x%x", fn, i, mTechLibNfcTypes[i], mTechParams[i].param.pa.sel_rsp);
  }
#if 1
  if(!mNumDiscNtf || NfcTag::getInstance().checkNextValidProtocol() == -1) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Check mIsSkipNotification = 0x%x", fn, mIsSkipNotification);
    if (!mIsSkipNotification) {
      ...
      if (e->ExceptionCheck()) {
        e->ExceptionClear();
        DLOG(ERROR) << StringPrintf("%s: fail notify nfc service", fn);
      }
    }
  } else {
    DLOG(ERROR) << StringPrintf("%s: Selecting next tag", fn);
  }
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit", fn);
#else // Removed AOSP code
  // notify NFC service about this new tag
  DLOG_IF(INFO, nfc_debug_enabled)
          ...
          TARGET_TYPE_ISO14443_4)  // is TagTechnology.ISO_DEP by Java API
      {
        if ((mTechParams[i].mode == NFC_DISCOVERY_TYPE_POLL_A) ||
          ...
          if (activationData.activate_ntf.intf_param.type ==
            ...
            // [SEC] Keep historical bytes for multi protocol tag
            gMulti_pa_iso = activationData.activate_ntf.intf_param.intf_param.pa_iso;
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          ...
          } else if (activationData.activate_ntf.intf_param.type ==
                     NFC_INTERFACE_MIFARE) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                "%s: T4T; Multi protocol; copy historical bytes; len=%u",
                fn, gMulti_pa_iso.his_byte_len);
            actBytes.reset(e->NewByteArray(gMulti_pa_iso.his_byte_len));
            if (gMulti_pa_iso.his_byte_len > 0)
              e->SetByteArrayRegion(actBytes.get(), 0, gMulti_pa_iso.his_byte_len,
                                    (jbyte*)(gMulti_pa_iso.his_byte));
          } else {
...
void NfcTag::resetTechnologies() {
  ...
  memset(mTechLibNfcTypes, 0, sizeof(mTechLibNfcTypes));
  memset(mTechParams, 0, sizeof(mTechParams));
  memset(&gMulti_pa_iso, 0, sizeof(gMulti_pa_iso));
  mIsDynamicTagId = false;
  mIsFelicaLite = false;
...
int NfcTag::checkNextValidProtocol(void) {
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, mNumDiscTechList=%x", fn, mNumDiscTechList);
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, gSelectedId=%x", fn, gSelectedId);

  for (int i = 0; i < mNumDiscTechList; i++) {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: nfa target idx=%d h=0x%X; protocol=0x%X", fn, i, mTechHandles [i], mTechLibNfcTypes [i]);

    if ((mTechHandles[gSelectedId] != mTechHandles [i]) &&
...
void NfcTag::selectNextTag ()
{
  ...
  if (foundIdx != -1) {
    ...
    if (stat == NFA_STATUS_OK) {
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: stat=%x; wait for activated ntf", fn, stat);
    } else {
      LOG(ERROR) << StringPrintf("%s: fail select; error=0x%X", fn, stat);
    }
  } else {
    LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: only found NFC-DEP technology.", fn);
  }
}
...
static uint16_t transProtocolValue(tNFC_PROTOCOL protocol) {
  if (protocol == NFC_PROTOCOL_T1T)      return DISC_PROTOCOL_T1T;
  if (protocol == NFC_PROTOCOL_T2T)      return DISC_PROTOCOL_T2T;
  if (protocol == NFC_PROTOCOL_T3T)      return DISC_PROTOCOL_T3T;
  if (protocol == NFC_PROTOCOL_ISO_DEP)  return DISC_PROTOCOL_ISO_DEP;
  if (protocol == NFC_PROTOCOL_NFC_DEP)  return DISC_PROTOCOL_NFC_DEP;
  if (protocol == NFC_PROTOCOL_MIFARE)   return DISC_PROTOCOL_MIFARE;
  if (protocol == NFC_PROTOCOL_B_PRIME)  return DISC_PROTOCOL_BPRIME;
  if (protocol == NFC_PROTOCOL_T5T)      return DISC_PROTOCOL_15693;
  if (protocol == NFC_PROTOCOL_KOVIO)    return DISC_PROTOCOL_KOVIO;

  return DISC_PROTOCOL_UNKNOWN;
}
...
void NfcTag::connectionEventHandler(uint8_t event, tNFA_CONN_EVT_DATA* data) {
  ...
  switch (event) {
    ...
    case NFA_ACTIVATED_EVT:
      ...
      gLastDiscoverProtocols = gDiscoverProtocols;
      break;

    case NFA_DEACTIVATED_EVT:
      mIsActivated = false;
      mProtocol = NFC_PROTOCOL_UNKNOWN;
      gDiscoverProtocols = 0x00;
      resetTechnologies();
      break;
