...
public class BluetoothPeripheralHandover implements BluetoothProfile.ServiceListener {
    ...
    public BluetoothPeripheralHandover(Context context, BluetoothDevice device, String name,
        ...
        Log.w(TAG, "Checking UUIDs & bt class");
        if (uuids != null) {
            Log.w(TAG, "Received UUIDs from NFC information");
            for (ParcelUuid uuid : uuids) {
                Log.w(TAG, "UUID: " + uuid.toString());
            }
        } else {
            Log.w(TAG, "No UUID received from NFC information");
        }
        if (btClass != null) {
            Log.w(TAG, "Received bt class from NFC information, btClass: "+ btClass.getDeviceClass());
        } else {
            Log.w(TAG, "No bt class received from NFC information");
        }
        ...
        if (!mIsHeadsetAvailable && !mIsA2dpAvailable) {
            ...
            Log.w(TAG, "Set availability for both Headset & A2DP");
        }
