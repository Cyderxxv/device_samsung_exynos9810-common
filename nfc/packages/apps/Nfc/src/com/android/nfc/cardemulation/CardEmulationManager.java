...
public class CardEmulationManager implements RegisteredServicesCache.Callback,
    ...
    public CardEmulationManager(Context context) {
        ...
        // [SEC] Get and set disabled service
        mServiceCache.setOthersDisabledSettings(getOthersDisabledSettings());
    ...
    // [SEC] Get disabled others services
    private List<ComponentName> getOthersDisabledSettings() {
        List<ComponentName> componentNames = new ArrayList<ComponentName>();
        String stringDisabled = Settings.Secure.getStringForUser(mContext.getContentResolver(),
                                                                 "nfc_others_disabled", UserHandle.getCallingUserId());
        if (stringDisabled != null) {
            String[] disabledComponents = stringDisabled.split("\\|");
            if (DBG) Log.d(TAG, "disabledComponents: " + disabledComponents.length);
            for (String stringComponent : disabledComponents) {
                ComponentName component = ComponentName.unflattenFromString(stringComponent);
    ...
    // [SEC] Get AID overflow status
    @Override
    public boolean onGetAidOverflow() {
        return mAidCache.isAidOverflow();
    }
    ...
    boolean setDefaultServiceForCategoryChecked(int userId, ComponentName service,
            String category) {
        if (!CardEmulation.CATEGORY_PAYMENT.equals(category)) {
            // [SEC] It will be used for toggle others service
            // Log.e(TAG, "Not allowing defaults for category " + category);
            setOthersChecked(userId, service);
            mServiceCache.setServiceDescription(userId, mServiceCache.getAidGroupDescription(userId, service));
            mServiceCache.invalidateCache(userId);
            if (mAidCache.isAidOverflow()) {
                setOthersChecked(userId, service);
                return false;
            }
            return true;
    ...
    private void setOthersChecked(int userId, ComponentName component) {
        mServiceCache.toggleOthersEnabledDisabled(component);
    }
    ...
    final class CardEmulationInterface extends INfcCardEmulation.Stub {
        ...
        public boolean isDefaultServiceForCategory(int userId, ComponentName service,
            ...
            // [SEC] Used as getting others service status
            ComponentName defaultService = null;
            if (category.equals(CardEmulation.CATEGORY_PAYMENT)) {
                defaultService = getDefaultServiceForCategory(userId, category, true);
            } else if (category.equals(CardEmulation.CATEGORY_OTHER)) {
                return !mServiceCache.isDisabledOthers(service);
            }
    ...
    @Override
    public boolean onPreferredPaymentServiceChanged(ComponentName service) { // [SEC] Return status to check AID overflow
        boolean isOverflow = false;
        int userId = ActivityManager.getCurrentUser();

        mServiceCache.setServiceDescription(userId, mServiceCache.getAidGroupDescription(userId, service));

        isOverflow = mAidCache.onPreferredPaymentServiceChanged(service);
        if (!isOverflow) {
            mHostEmulationManager.onPreferredPaymentServiceChanged(service);
        }

        return !isOverflow;
        // [SEC] Comment out AOSP
