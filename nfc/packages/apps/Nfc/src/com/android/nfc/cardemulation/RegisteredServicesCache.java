...
public class RegisteredServicesCache {
    ...
    // [SEC] Disabled Others service cache
    final List<ComponentName> mDisabledComponentName;

    public interface Callback {
        void onServicesUpdated(int userId, final List<ApduServiceInfo> services);
        boolean onGetAidOverflow(); // [SEC] Get AID Overflow state
    };
    ...
    private static class UserServices {
        ...
        final HashMap<ComponentName, String> description =
                Maps.newHashMap(); // [SEC] Service description
    };
    ...
    public RegisteredServicesCache(Context context, Callback callback) {
                ...
                if (uid != -1) {
                    ...
                    if (!replaced) {
                        ...
                        if (currentUser == UserHandle.getUserId(uid)) {
                            // [SEC] Set current service description
                            ApduServiceInfo addedService = null;
                            if (Intent.ACTION_PACKAGE_ADDED.equals(action)) {
                                addedService = getAddedService(UserHandle.getUserId(uid),
                                                               intent.getData().getSchemeSpecificPart());
                                if (addedService == null) {
                                    return;
                                }
                                setServiceDescription(UserHandle.getUserId(uid),
                                                      getAidGroupDescription(UserHandle.getUserId(uid),
                                                      addedService.getComponent()));
                            }

                            invalidateCache(UserHandle.getUserId(uid));

                            // [SEC] Handle AID Overflow
                            if (mCallback.onGetAidOverflow()) {
                                if (addedService != null) {
                                    toggleOthersEnabledDisabled(addedService.getComponent());
                                }
                                invalidateCache(UserHandle.getUserId(uid));
                            }
                        } else {
        ...
        mDisabledComponentName = new ArrayList<ComponentName>(); // [SEC] Disabled Others service cache
    }
        ...
        synchronized (mLock) {
            ...
            while (it.hasNext()) {
                ...
                if (!containsServiceLocked(validServices, entry.getKey())) {
                    ...
                    it.remove();
                    // [SEC] Remove from disabled service
                    removeOthersDisabledCache(entry.getKey());
                    // [SEC] Remove service description
                    userServices.description.remove(entry.getKey());
                }
            }
            for (ApduServiceInfo service : validServices) {
                ...
                // [SEC] Set service description
                userServices.description.put(service.getComponent(),
                                             getAidGroupDescription(userId, service.getComponent()));
            }
            ...
            if (toBeRemoved.size() > 0) {
                for (ComponentName component : toBeRemoved) {
                    ...
                    // [SEC] Remove from disabled service
                    removeOthersDisabledCache(component);
                    // [SEC] Remove service description
                    userServices.description.remove(component);
                }
                // Persist to filesystem
                writeDynamicSettingsLocked();
            }
            // [SEC] Save description for settings
            saveOthersDescription(userId);
            // [SEC] remove disabled entry from validservices
            ArrayList<ApduServiceInfo> toBeRemovedValidService = new ArrayList<ApduServiceInfo>();
            for (ApduServiceInfo service : validServices) {
                if (isDisabledOthers(service.getComponent())) {
                    toBeRemovedValidService.add(service);
                }
            }
            if (toBeRemovedValidService.size() > 0) {
                for (ApduServiceInfo serviceInfo : toBeRemovedValidService) {
                    Log.d(TAG, "Remove from validservices : " + serviceInfo.getComponent());
                    validServices.remove(serviceInfo);
                }
            }
        }
    ...
    public boolean registerAidGroupForService(int userId, int uid,
        ...
        synchronized (mLock) {
            ...
            // [SEC] remove disabled service
            ArrayList<ApduServiceInfo> toBeRemovedNewService = new ArrayList<ApduServiceInfo>();
            for (ApduServiceInfo service : newServices) {
                if (isDisabledOthers(service.getComponent())) {
                    toBeRemovedNewService.add(service);
                }
            }
            try {
                if (toBeRemovedNewService.size() > 0) {
                    for (ApduServiceInfo service : newServices) {
                        Log.d(TAG, "Remove from validservices: " + service.getComponent());
        ...
        if (success) {
            // [SEC] Set description
            setServiceDescription(userId, getAidGroupDescription(userId, componentName));
            // Make callback without the lock held
            mCallback.onServicesUpdated(userId, newServices);
            // [SEC] AID Overflow handle
            if (mCallback.onGetAidOverflow()) {
                Log.e(TAG, "AID Oveflow!!!!");
                return false;
            }
        }
        return success;
    ...
    public boolean removeAidGroupForService(int userId, int uid, ComponentName componentName,
        ...
        synchronized (mLock) {
            ...
            if (serviceInfo != null) {
                ...
                // [SEC] remove from disabled cache
                removeOthersDisabledCache(componentName);
            } else {
                Log.e(TAG, "Service " + componentName + " does not exist.");
            }
        }
    ...
    // [SEC] Save disabled others service cache to string for Settings
    private void saveOthersDisabledPreferences() {
        String stringDisabledComponents = "";
        for (ComponentName disableComponent : mDisabledComponentName) {
            if (disableComponent != null) {
                stringDisabledComponents += (disableComponent.flattenToString() + "|");
            }
        }
        Settings.Secure.putStringForUser(mContext.getContentResolver(),
                                         "nfc_others_disabled", stringDisabledComponents,
                                         UserHandle.getCallingUserId());
    }

    // [SEC] Remove others service from cache
    private void removeOthersDisabledCache(ComponentName component) {
        ArrayList<ComponentName> toBeRemoved = new ArrayList<ComponentName>();
        for (ComponentName disableComponent : mDisabledComponentName) {
            if (disableComponent!= null && disableComponent.equals(component)) {
                toBeRemoved.add(disableComponent);
            }
        }
        if (toBeRemoved.size() > 0) {
            for (ComponentName removeComponent : toBeRemoved) {
                mDisabledComponentName.remove(removeComponent);
            }
        }
        saveOthersDisabledPreferences();
    }

    // [SEC] Check enabled service
    public boolean isDisabledOthers(ComponentName component) {
        for (ComponentName disabledComponent : mDisabledComponentName) {
            if (component.equals(disabledComponent)) {
                Log.d(TAG, "Others service disabled : " + component);
                return true;
            }
        }
        Log.d(TAG, "Others service enabled : " + component);
        return false;
    }

    // [SEC] Toggle others service to enable or disable
    public void toggleOthersEnabledDisabled(ComponentName component) {
        boolean foundOthers = false;
        for (ComponentName disabledComponent : mDisabledComponentName) {
            if (disabledComponent != null && disabledComponent.equals(component)) {
                mDisabledComponentName.remove(disabledComponent);
                foundOthers = true;
                break;
            }
        }
        if (!foundOthers) {
            mDisabledComponentName.add(component);
        }
        saveOthersDisabledPreferences();
    }

    // [SEC] Save and load enabled other service
    public void setOthersDisabledSettings(List<ComponentName> componentNames) {
        mDisabledComponentName.clear();
        for (ComponentName component : componentNames) {
            mDisabledComponentName.add(component);
        }
    }

    // [SEC] Get added service info
    private ApduServiceInfo getAddedService(int userId, String packageName) {
        ArrayList<ApduServiceInfo> validServices = getInstalledServices(userId);
        if (validServices == null) {
            return null;
        }
        for (ApduServiceInfo service : validServices) {
            if (packageName.equals(service.getComponent().getPackageName())) {
                return service;
            }
        }
        return null;
    ...
    // [SEC] Set current service description
    public void setServiceDescription(int userId, String description) {
        Settings.Secure.putStringForUser(mContext.getContentResolver(),
                                         "nfc_service_desc", description, userId);
    }

    // [SEC] Get AID Group description
    public String getAidGroupDescription(int userId, ComponentName component) {
        UserServices userServices = findOrCreateUserLocked(userId);
        String description;

        // Get from cache
        description = userServices.description.get(component);
        if (description != null) {
            return description;
        }
        ...
        try {
            Bundle metaData = mContext.getPackageManager().getServiceInfo(component, PackageManager.GET_META_DATA).metaData;
            if (metaData != null) {
                Resources res = mContext.getPackageManager().getResourcesForApplication(component.getPackageName());
                for (String key : metaData.keySet()) {
                    XmlResourceParser parser = res.getXml(metaData.getInt(key));
                    parser.next();
                    int eventType = parser.getEventType();
                    while (eventType != XmlPullParser.END_DOCUMENT) {
                        if (parser.getName() != null && parser.getName().equals("aid-group")) {
                            for (int i = 0; i < parser.getAttributeCount(); i++) {
                                if (parser.getAttributeName(i).equals("description")) {
                                    int resId = Integer.parseInt(parser.getAttributeValue(0).substring(1));
    ...
    // [SEC] Save others description for Settings
    private void saveOthersDescription(int userId) {
        UserServices userServices = findOrCreateUserLocked(userId);
        String descriptionSet = "";
        String desc = null;

        for (Map.Entry<ComponentName, String> entry : userServices.description.entrySet()) {
            desc = (entry.getKey().flattenToString() + ":" + entry.getValue() + ",");
            descriptionSet += desc;
        }

        Settings.Secure.putStringForUser(mContext.getContentResolver(),
                                         "nfc_others_desc", descriptionSet,
                                         userId);
    }
}
